...
#include "acl/acl.h"
#include "../../op_host/aclnn_scatter.h"
...
int main() {
  ...
  size_t indexShapeSize = GetShapeSize(indexShape);
  size_t srcShapeSize = GetShapeSize(srcShape);
  size_t dtypeSize = sizeof(float);
  size_t indexSize = sizeof(int64_t);
  size_t srcSize = sizeof(float);
  ...
  void **inputIndex = (void **)(&indexHostData);
  void **inputSrc = (void **)(&srcHostData);

  // 设定数据
  size_t fileSize;

  ReadFile("../input/input_self.bin", fileSize, *inputSelf, selfShapeSize * dtypeSize);
  ReadFile("../input/input_index.bin", fileSize, *inputIndex, indexShapeSize * indexSize);
  ReadFile("../input/input_src.bin", fileSize, *inputSrc, srcShapeSize * srcSize);
  ...
  //写出数据
  void ** out_values=(void **)(&resultData);
  WriteFile("../output/output_values.bin", *out_values, size * dtypeSize);
