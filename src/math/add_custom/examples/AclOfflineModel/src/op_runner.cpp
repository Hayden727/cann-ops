...
bool OpRunner::Init()
{
    for (size_t i = 0; i < numInputs_; ++i) {
        ...
        if (g_isDevice) {
            if (aclrtMalloc(&hostMem, size, ACL_MEM_MALLOC_HUGE_FIRST) != ACL_SUCCESS) {
                ERROR_LOG("Malloc device memory for input[%zu] failed", i);
                return false;
            }
        } else {
...
bool OpRunner::RunOp()
{
    ...
    auto ret = aclopExecuteV2(opDesc_->opType.c_str(), numInputs_, opDesc_->inputDesc.data(), inputBuffers_.data(),
                              numOutputs_, opDesc_->outputDesc.data(), outputBuffers_.data(), opDesc_->opAttr, stream);
