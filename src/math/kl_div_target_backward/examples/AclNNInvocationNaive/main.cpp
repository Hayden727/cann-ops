...
int main(int argc, char **argv)
{
    ...
    size_t inputX1ShapeSize = inputX1Shape[0] * inputXShape[1];
    size_t outputZShapeSize = outputZShape[0] * outputZShape[1];
    std::vector<aclFloat16> inputX0HostData(inputXShapeSize);
    std::vector<aclFloat16> inputX1HostData(inputX1ShapeSize);
    std::vector<aclFloat16> inputX2HostData(inputXShapeSize);
    ...
    // 创建inputX aclTensor
    ret = CreateAclTensor(inputX0HostData, inputXShape, &inputX0DeviceAddr, aclDataType::ACL_FLOAT16, &inputX0);
    ...
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    ret = CreateAclTensor(inputX2HostData, inputXShape, &inputX2DeviceAddr, aclDataType::ACL_FLOAT16, &inputX2);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建outputZ aclTensor
    ...
    // 计算workspace大小并申请内存
    ret = aclnnKlDivTargetBackwardGetWorkspaceSize(inputX0, inputX1, inputX2, 0, true, outputZ, &workspaceSize, &executor);
