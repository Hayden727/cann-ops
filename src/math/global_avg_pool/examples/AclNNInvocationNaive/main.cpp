...
#include "acl/acl.h"
#include "aclnn_global_avg_pool.h"
...
int main(int argc, char **argv)
{
    ...
    std::vector<int64_t> inputXShape = {3, 4, 64, 32};
    ...
    // 计算workspace大小并申请内存
    ret = aclnnGlobalAvgPoolGetWorkspaceSize(inputX, outputY, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnGlobalAvgPoolGetWorkspaceSize failed. ERROR: %d\n", ret); return FAILED);
    ...
    // 执行算子
    ret = aclnnGlobalAvgPool(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnGlobalAvgPool failed. ERROR: %d\n", ret); return FAILED);
