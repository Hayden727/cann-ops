...
#include "acl/acl.h"
#include "aclnn_addcmul_sample.h"
...
int main(int argc, char **argv)
{
    ...
    std::vector<int64_t> inputDataShape = {100, 100};
    std::vector<int64_t> inputX1Shape = {100, 100};
    std::vector<int64_t> inputX2Shape = {100, 100};
    std::vector<int64_t> inputValueShape = {1};
    std::vector<int64_t> outputYShape = {100, 100};
    void *inputDataDeviceAddr = nullptr;
    ...
    std::vector<aclFloat16> inputValueHostData(1);
    ...
    ReadFile("../input/input_x2.bin", fileSize, *input3, inputX2ShapeSize_1*dataType);
    ReadFile("../input/input_value.bin", fileSize, *input4, inputValueShapeSize_1*dataType);
    INFO_LOG("Set input success");
    // 创建input aclTensor
    ret = CreateAclTensor(inputDataHostData, inputDataShape, &inputDataDeviceAddr, aclDataType::ACL_FLOAT16, &inputData);
    ...
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    ret = CreateAclTensor(inputValueHostData, inputValueShape, &inputValueDeviceAddr, aclDataType::ACL_FLOAT16, &inputValue);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    ...
    ret = aclnnAddcmulSampleGetWorkspaceSize(inputData, inputX1, inputX2, inputValue, outputY, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAddcmulSampleGetWorkspaceSize failed. ERROR: %d\n", ret); return FAILED);
    ...
    // 执行算子
    ret = aclnnAddcmulSample(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAddcmulSample failed. ERROR: %d\n", ret); return FAILED);
