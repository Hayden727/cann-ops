...
- 算子功能：执行 x1 与 x2 的逐元素乘法，将结果乘以value并与输入input_data做逐元素加法。
...
调用`Ascend C`的`API`接口`Mul`和`Add`进行实现。


## 算子执行接口

每个算子分为两段式接口，必须先调用“aclnnAddcmulSampleGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnAddcmulSample”接口执行计算。

* `aclnnStatus aclnnAddcmulSampleGetWorkspaceSize(const aclTensor* inputData, const aclTensor* x1, const aclTensor* x2, const aclTensor* value, aclTensor* out, uint64_t* workspaceSize, aclOpExecutor** executor)`
* `aclnnStatus aclnnAddcmulSample(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor, aclrtStream stream)`
...
### aclnnAddcmulSampleGetWorkspaceSize

- **参数说明：**

  - inputData（aclTensor\*，计算输入）：必选参数，Device侧的aclTensor，公式中的输入input_data，数据类型支持FLOAT32、FLOAT16、INT32、INT8，数据格式支持ND。
  ...
  - value（aclTensor\*，计算输入）：必选参数，Device侧的aclTensor，公式中的输入value，数据类型支持FLOAT32、FLOAT16、INT32、INT8，数据格式支持ND。
  - out（aclTensor\*，计算输出）：Device侧的aclTensor，公式中的输出y，数据类型支持FLOAT32、FLOAT16、INT32、INT8，数据格式支持ND。
...
- **参数说明：**
  ...
  - workspaceSize（uint64\_t，入参）：在Device侧申请的workspace大小，由第一段接口aclnnAddcmulSampleGetWorkspaceSize获取。
...
- inputData，x1，x2，value，out的数据类型支持FLOAT32、FLOAT16、INT32、INT8，数据格式只支持ND
...
详见[AddcmulSample自定义算子样例说明算子调用章节](../README.md#算子调用)