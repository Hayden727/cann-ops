...
int main(int argc, char **argv)
{
    ...
    std::vector<int64_t> inputDyShape = {1024, 1024};
    std::vector<int64_t> inputXShape = {1024, 1024};
    std::vector<int64_t> inputYShape = {1024, 1024};
    std::vector<int64_t> outputZShape = {1024, 1024};
    void *inputDyDeviceAddr = nullptr;
    void *inputXDeviceAddr = nullptr;
    void *inputYDeviceAddr = nullptr;
    void *outputZDeviceAddr = nullptr;
    aclTensor *inputDy = nullptr;
    aclTensor *inputX = nullptr;
    aclTensor *inputY = nullptr;
    aclTensor *outputZ = nullptr;
    size_t inputDyShapeSize_1=inputDyShape[0] * inputDyShape[1];
    size_t inputXShapeSize_1=inputXShape[0] * inputXShape[1];
    size_t inputYShapeSize_1=inputYShape[0] * inputYShape[1];
    size_t outputZShapeSize_1=outputZShape[0] * outputZShape[1];
    size_t dataType=2;
    std::vector<aclFloat16> inputDyHostData(inputDyShape[0] * inputDyShape[1]);
    ...
    ReadFile("../input/input_y.bin", fileSize, *input3, inputXShapeSize_1*dataType);

    INFO_LOG("Set input success");
    // 创建inputX aclTensor
    ret = CreateAclTensor(inputDyHostData, inputDyShape, &inputDyDeviceAddr, aclDataType::ACL_FLOAT16, &inputDy);
    ...
    // 计算workspace大小并申请内存
    ret = aclnnGeluGradGetWorkspaceSize(inputDy, inputX, inputY, outputZ, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnGeluGradGetWorkspaceSize failed. ERROR: %d\n", ret); return FAILED);
    ...
    // 执行算子
    ret = aclnnGeluGrad(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnGeluGrad failed. ERROR: %d\n", ret); return FAILED);
    ...
    aclDestroyTensor(inputDy);
    aclDestroyTensor(inputX);
    aclDestroyTensor(inputY);
    aclDestroyTensor(outputZ);

    // 7. 释放device资源，需要根据具体API的接口定义修改
    aclrtFree(inputDyDeviceAddr);
