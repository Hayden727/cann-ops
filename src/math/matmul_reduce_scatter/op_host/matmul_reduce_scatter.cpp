...
static ge::graphStatus MatmulReduceScatterTilingFunc(gert::TilingContext *context) {
    ...
    MatmulReduceScatterTilingData *tiling = context->GetTilingData<MatmulReduceScatterTilingData>();
    ...
    // matmul tile tiling
    if (!matmulTilingFunc(TILE_M / RANK_NUM, N, K, tiling->matmulTiling)) {
        ERROR_LOG("Get tile matmul tiling failed");
        return ge::GRAPH_FAILED;
    }
    // matmul tail tiling
    if ( M % TILE_M != 0U) {
        if (!matmulTilingFunc(tailM / RANK_NUM, N, K, tiling->tailTiling)) {
...
public:
    explicit MatmulReduceScatter(const char *name) : OpDef(name)
    {
        ...
        this->AICore().SetTiling(MatmulReduceScatterTilingFunc);
