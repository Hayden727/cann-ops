...
int main(int argc, char **argv)
{
    ...
    std::vector<int64_t> inputAShape = {1024, 256};
    std::vector<int64_t> inputBShape = {256, 640};
    std::vector<int64_t> inputBiasShape = {640};
    std::vector<int64_t> outputCShape = {1024, 640};

    void *inputADeviceAddr = nullptr;
    void *inputBDeviceAddr = nullptr;
    void *inputBiasDeviceAddr = nullptr;
    ...
    size_t inputAShapeSize_1=inputAShape[0] * inputAShape[1];
    size_t inputBShapeSize_1=inputBShape[0] * inputBShape[1];
    ...
    size_t fileSize = 0;
    void** input1=(void**)(&inputAHostData);
    void** input2=(void**)(&inputBHostData);
    void** input3=(void**)(&inputBiasHostData);

    //读取数据
    size_t dataType=2;
    ReadFile("../input/input_a.bin", fileSize, *input1, inputAShapeSize_1 * dataType);
    ReadFile("../input/input_b.bin", fileSize, *input2, inputBShapeSize_1 * dataType);
    ReadFile("../input/input_bias.bin", fileSize, *input3, inputBiasShapeSize_1 * sizeof(float));
    INFO_LOG("Set input success");

    // 创建inputA aclTensor
    ret = CreateAclTensor(inputAHostData, inputAShape, &inputADeviceAddr, aclDataType::ACL_FLOAT16, &inputA);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建inputB aclTensor
    ret = CreateAclTensor(inputBHostData, inputBShape, &inputBDeviceAddr, aclDataType::ACL_FLOAT16, &inputB);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建inputBias aclTensor
    ret = CreateAclTensor(inputBiasHostData, inputBiasShape, &inputBiasDeviceAddr, aclDataType::ACL_FLOAT, &inputBias);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建outputC aclTensor
    ret = CreateAclTensor(outputCHostData, outputCShape, &outputCDeviceAddr, aclDataType::ACL_FLOAT, &outputC);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    ...
    auto size = GetShapeSize(outputCShape);
    ...
    //写出数据
    WriteFile("../output/output_c.bin", *output1, outputCShapeSize_1 * sizeof(float));
