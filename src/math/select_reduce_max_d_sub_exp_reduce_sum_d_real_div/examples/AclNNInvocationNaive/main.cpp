...
#include "acl/acl.h"
#include "aclnn_select_reduce_max_d_sub_exp_reduce_sum_d_real_div.h"
...
int main(int argc, char **argv)
{
    ...
    std::vector<aclFloat16> input_3_host_data(input_3_shape[0] * input_3_shape[1] * input_3_shape[dim_2]);
    std::vector<aclFloat16> output_host_data(output_shape[0] * output_shape[1] * output_shape[dim_2]);

    ReadFile("../input/input_1.bin", 0, input_1_host_data.data(), input_1_host_data.size() * 1);
    ...
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    ret = CreateAclTensor(input_3_host_data, input_3_shape, &input_3_device, aclDataType::ACL_FLOAT16, &input_3);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    ...
    // 计算workspace大小并申请内存
    ret = aclnnSelectReduceMaxDSubExpReduceSumDRealDivGetWorkspaceSize(input_1, input_2, input_3, output, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnSelectReduceMaxDSubExpReduceSumDRealDivGetWorkspaceSize failed. ERROR: %d\n", ret); return FAILED);
    ...
    // 执行算子
    ret = aclnnSelectReduceMaxDSubExpReduceSumDRealDiv(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnSelectReduceMaxDSubExpReduceSumDRealDiv failed. ERROR: %d\n", ret); return FAILED);
