...
int main() {
  ...
  std::vector<int64_t> xShape = {10, 10};
  std::vector<int64_t> outShape = {10, 10};
  using xDtype = float;
  using outDtype = int32_t;
  size_t xShapeSize = GetShapeSize(xShape);
  size_t xDtypeSize = sizeof(xDtype);
  size_t outDtypeSize = sizeof(outDtype);
  void* xDeviceAddr = nullptr;
  void* outDeviceAddr = nullptr;
  aclTensor* x = nullptr;
  aclTensor* out = nullptr;
  std::vector<xDtype> xHostData(100, 0);
  ...
  // 设定数据
  size_t fileSize;
  ReadFile("../input/input_x.bin", fileSize, *inputX, xShapeSize * xDtypeSize);
  // 创建x aclTensor
  ...
  // 调用aclnnCast第一段接口
  ret = aclnnCastGetWorkspaceSize(x, dstType, out, &workspaceSize, &executor);
  ...
  std::vector<outDtype> outData(outSize, 0);
  ...
  //写出数据
  void ** out_values=(void **)(&outData);
  WriteFile("../output/output_values.bin", *out_values, outSize * outDtypeSize);
