...
自定义算子编译部署后，会自动生成单算子API，可以直接在应用程序中调用。算子API的形式一般定义为“两段式接口”，形如：
   ```cpp    
   aclnnStatus aclnnAddcmulGetWorkspaceSize(const aclTensor *input_data, const aclTensor *x1, const aclTensor *x2, const aclScalar *value, const aclTensor *out, uint64_t workspaceSize, aclOpExecutor **executor);
...
**请确保已根据算子包编译部署步骤完成本算子的编译部署动作。**
  ...
  - 进入样例代码所在路径
    ...
    ```bash
    cd ${git_clone_path}/ops-contribution/src/math/addcmul/examples/AclNNInvocationNaive
    ```
