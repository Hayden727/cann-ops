...
int main(int argc, char **argv)
{
    ...
    std::vector<float> inputX1HostData(inputX1ShapeSize);
    std::vector<float> inputX2HostData(inputX2ShapeSize);
    std::vector<uint8_t> outputYHostData(outputYShapeSize);
    size_t dataType = sizeof(float);
    size_t fileSize = 0;
    void * input1=&inputX1HostData[0];
    void * input2=&inputX2HostData[0];
    //读取数据
    ...
    // 计算workspace大小并申请内存
    ret = aclnnLessEqualGetWorkspaceSize(inputX1, inputX2, outputY, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnLessEqualGetWorkspaceSize failed. ERROR: %d\n", ret); return FAILED);
    ...
    // 执行算子
    ret = aclnnLessEqual(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnLessEqual failed. ERROR: %d\n", ret); return FAILED);
