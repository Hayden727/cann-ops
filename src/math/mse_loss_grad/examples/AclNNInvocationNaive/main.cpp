...
int main(int argc, char **argv)
{
    ...
    aclTensor *inputPredict = nullptr;
    aclTensor *inputLabel = nullptr;
    aclTensor *inputDout = nullptr;
    ...
    void** input3=(void**)(&inputDoutHostData);
    ...
    ReadFile("../input/input_dout.bin", fileSize, *input3, inputDoutShapeSize_1 * sizeof(float));
    ...
    // 创建inputLabel aclTensor
    ret = CreateAclTensor(inputLabelHostData, inputLabelShape, &inputLabelDeviceAddr, aclDataType::ACL_FLOAT, &inputLabel);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建inputDout aclTensor
    ret = CreateAclTensor(inputDoutHostData, inputDoutShape, &inputDoutDeviceAddr, aclDataType::ACL_FLOAT, &inputDout);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建outputY aclTensor
    ...
    aclDestroyTensor(inputPredict);
    aclDestroyTensor(inputLabel);
    aclDestroyTensor(inputDout);
    aclDestroyTensor(outputY);

    // 7. 释放device资源，需要根据具体API的接口定义修改
    aclrtFree(inputPredictDeviceAddr);
    aclrtFree(inputLabelDeviceAddr);
    aclrtFree(inputDoutDeviceAddr);
