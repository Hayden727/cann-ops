...
```
├── msopst.ini                      // st测试配置文件 
├── UnalignAddCustom_case_all_type.json    // 测试用例定义文件示例(8.0.RC3.alpha003版本生成)
└── test_unalign_add_custom.py              // 算子期望数据生成脚本
```
...
## 执行测试用例
  **请确保已根据算子包编译部署步骤完成本算子的编译部署动作。**
  ...
  - 进入到测试用例目录
    ...
    ```bash
    cd ${git_clone_path}/cann-ops/src/math/unalign_add_custom/tests/st
    ```
  ...
  - 执行测试用例
    ...
    ```bash
    ${INSTALL_DIR}/python/site-packages/bin/msopst run -i ./UnalignAddCustom_case_all_type.json -soc {Soc Version} -out ./output -conf msopst.ini
    ```
