...
def verify_result(cal_result, golden):
    ...
    cal_result = np.fromfile(cal_result, dtype=res_type).reshape(
        (dim_size, dim_size)
    )  # 从bin文件读取实际运算结果
    golden = np.fromfile(golden, dtype=res_type).reshape(
        (dim_size, dim_size)
    )  # 从bin文件读取预期运算结果
    ...
    if not result_rtol.all() and not result_atol.all():
        if (
            np.sum(result_rtol == False) > cal_result.size * LOSS
            and np.sum(result_atol == False) > cal_result.size * LOSS
