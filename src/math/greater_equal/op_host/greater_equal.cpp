...
struct GreaterEqualTilingParam {
...
static void ProcessGreater32B(GreaterEqualTilingParam& param) {
...
static void ProcessGreaterEqual(GreaterEqualTilingParam& param, uint32_t coreNum) {
...
static ge::graphStatus TilingFunc(gert::TilingContext* context) {
  ...
  uint32_t quotient = 0;
  uint32_t remainder = param.padMax;
  while (remainder >= dataSize) {
    remainder -= dataSize;
    quotient++;
  }
  param.padMax = quotient;
  ...
  if (param.totalLength < param.pad32) {
    ...
    ProcessGreaterEqual(param, coreNum);
  }
  tiling.set_totalLength(param.totalLength);
