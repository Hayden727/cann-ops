...
GreaterEqual由GreaterEqual操作组成，计算过程只有1步：

1. out = GreaterEqual(x1[offset], x2[offset])
...
### aclnnGreaterEqualGetWorkspaceSize
...
- **参数说明：**
  ...
  - workspaceSize（uint64\_t，入参）：在Device侧申请的workspace大小，由第一段接口aclnnGreaterEqualGetWorkspaceSize获取。
...
详见[GreaterEqual自定义算子样例说明算子调用章节](../README.md#算子调用)
