...
#include "acl/acl.h"
#include "aclnn_pre_layer_norm.h"
...
int main(int argc, char **argv)
{
    ...
    if (strcmp(argv[1], "case1") == 0) {
        ...
        inputXShape = {512, 4, 20480};  // case2的新shapeX
        inputYShape = {512, 4, 20480};  // case2的新shapeY
        eps = 1e-7;
    } else {
    ...
    std::vector<int64_t> inputGammaShape = {inputXShape[2]};
    std::vector<int64_t> inputBetaShape = {inputXShape[2]};
    std::vector<int64_t> outputZShape = {inputXShape[0], inputXShape[1], inputXShape[2]};
    ...
    size_t inputXShapeSize_1 = inputXShape[0] * inputXShape[1] * inputXShape[2];
    size_t inputYShapeSize_1 = inputYShape[0] * inputYShape[1] * inputYShape[2];
    size_t inputGammaShapeSize_1 = inputGammaShape[0];
    size_t inputBetaShapeSize_1 = inputBetaShape[0];
    size_t outputYShapeSize_1= inputXShape[0] * inputXShape[1] * inputXShape[2];
    size_t dataType=4; // float
    std::vector<float> inputXHostData(inputXShape[0] * inputXShape[1] * inputXShape[2]);
    std::vector<float> inputYHostData(inputYShape[0] * inputYShape[1] * inputYShape[2]);
    std::vector<float> inputGammaHostData(inputGammaShape[0]);
    std::vector<float> inputBetaHostData(inputBetaShape[0]);
    std::vector<float> outputZHostData(outputZShape[0] * outputZShape[1] * outputZShape[2]);

    size_t fileSize = 0;
    void** input1=(void**)(&inputXHostData);
    void** input2=(void**)(&inputYHostData);
    void** input3=(void**)(&inputGammaHostData);
    void** input4=(void**)(&inputBetaHostData);
    //读取数据
    ...
    ReadFile("../input/input_gamma.bin", fileSize, *input3, inputGammaShapeSize_1*dataType);
    ReadFile("../input/input_beta.bin", fileSize, *input4, inputBetaShapeSize_1*dataType);
    ...
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建inputGamma aclTensor
    ret = CreateAclTensor(inputGammaHostData, inputGammaShape, &inputGammaDeviceAddr, aclDataType::ACL_FLOAT, &inputGamma);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建inputBeta aclTensor
    ret = CreateAclTensor(inputBetaHostData, inputBetaShape, &inputBetaDeviceAddr, aclDataType::ACL_FLOAT, &inputBeta);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建outputZ aclTensor
    ...
    //写出数据
    WriteFile("../output/output_z.bin", *output1, outputYShapeSize_1*dataType);
