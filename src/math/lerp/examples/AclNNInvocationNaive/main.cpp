...
int main(int argc, char **argv)
{
    ...
    std::vector<int64_t> inputStartShape = {32};
    std::vector<int64_t> inputEndShape = {32};
    std::vector<int64_t> inputWeightShape = {32};
    std::vector<int64_t> outputYShape = {32};
    void *inputStartDeviceAddr = nullptr;
    void *inputEndDeviceAddr = nullptr;
    void *inputWeightDeviceAddr = nullptr;
    void *outputYDeviceAddr = nullptr;
    aclTensor *inputStart = nullptr;
    aclTensor *inputEnd = nullptr;
    aclTensor *inputWeight = nullptr;
    ...
    std::vector<aclFloat16> inputWeightHostData(inputWeightShape[0]);
    std::vector<aclFloat16> outputYHostData(outputYShape[0]);
    ...
    ReadFile("../input/input_weight.bin", fileSize, *input3, inputWeightShapeSize_1*dataType);
    INFO_LOG("Set input success");
    // 创建inputStart aclTensor
    ret = CreateAclTensor(inputStartHostData, inputStartShape, &inputStartDeviceAddr, aclDataType::ACL_FLOAT16, &inputStart);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建inputEnd aclTensor
    ret = CreateAclTensor(inputEndHostData, inputEndShape, &inputEndDeviceAddr, aclDataType::ACL_FLOAT16, &inputEnd);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建inputWeight aclTensor
    ret = CreateAclTensor(inputWeightHostData, inputWeightShape, &inputWeightDeviceAddr, aclDataType::ACL_FLOAT16, &inputWeight);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建outputY aclTensor
    ...
    // 计算workspace大小并申请内存
    ret = aclnnLerpGetWorkspaceSize(inputStart, inputEnd, inputWeight, outputY, &workspaceSize, &executor);
    ...
    aclDestroyTensor(inputEnd);
    aclDestroyTensor(inputWeight);
    aclDestroyTensor(outputY);

    // 7. 释放device资源，需要根据具体API的接口定义修改
    aclrtFree(inputStartDeviceAddr);
    aclrtFree(inputEndDeviceAddr);
    aclrtFree(inputWeightDeviceAddr);
