...
- 算子功能：对两个张量以`start`，`end`做线性插值，将结果返回到输出张量。
...
Lerp算子通过依次调用`Ascend C`的`API`接口：`Sub`、`Mul`、`Add`，分步计算实现Lerp函数计算。对于16位的数据类型将其先通过`Cast`接口转换为32位浮点数后，再使用`Sub`、`Mul`、`Add`进行计算。
