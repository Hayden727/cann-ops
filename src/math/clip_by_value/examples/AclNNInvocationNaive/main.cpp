...
int main(int argc, char **argv)
{
    ...
    std::vector<int64_t> inputXShape = {7, 2045};
    std::vector<int64_t> inputClipValueMinShape = {1};
    std::vector<int64_t> inputClipValueMaxShape = {1};
    std::vector<int64_t> outputYShape = {7, 2045};
    void *inputXDeviceAddr = nullptr;
    void *inputClipValueMinDeviceAddr = nullptr;
    void *inputClipValueMaxDeviceAddr = nullptr;
    void *outputYDeviceAddr = nullptr;
    aclTensor *inputX = nullptr;
    aclTensor *inputClipValueMin = nullptr;
    ...
    // 创建inputClipValueMin aclTensor
    ret = CreateAclTensor(inputClipValueMinHostData, inputClipValueMinShape, &inputClipValueMinDeviceAddr, aclDataType::ACL_FLOAT16, &inputClipValueMin);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建inputClipValueMax aclTensor
    ret = CreateAclTensor(inputClipValueMaxHostData, inputClipValueMaxShape, &inputClipValueMaxDeviceAddr, aclDataType::ACL_FLOAT16, &inputClipValueMax);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建outputY aclTensor
    ...
    // 计算workspace大小并申请内存
    ret = aclnnClipByValueGetWorkspaceSize(inputX, inputClipValueMin, inputClipValueMax, outputY, &workspaceSize, &executor);
    ...
    // 执行算子
    ret = aclnnClipByValue(workspaceAddr, workspaceSize, executor, stream);
    ...
    aclDestroyTensor(inputClipValueMin);
    aclDestroyTensor(inputClipValueMax);
    aclDestroyTensor(outputY);

    // 7. 释放device资源，需要根据具体API的接口定义修改
    aclrtFree(inputXDeviceAddr);
    aclrtFree(inputClipValueMinDeviceAddr);
    aclrtFree(inputClipValueMaxDeviceAddr);
