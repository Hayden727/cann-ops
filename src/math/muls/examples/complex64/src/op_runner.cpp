...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnMulsGetWorkspaceSize(inputTensor_[0], value,outputTensor_[0],
	...
	INFO_LOG("Execute aclnnMulsGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnMuls(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
	INFO_LOG("Execute aclnnMuls success");
