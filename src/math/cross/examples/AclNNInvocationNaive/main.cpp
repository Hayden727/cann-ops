...
int main(int argc, char **argv)
{
    ...
    int64_t dim = 1;
    std::vector<int32_t> inputX1HostData(inputX1Shape[0] * inputX1Shape[1]);
    std::vector<int32_t> inputX2HostData(inputX2Shape[0] * inputX2Shape[1]);
    ...
    ret = CreateAclTensor(inputX1HostData, inputX1Shape, &inputX1DeviceAddr, aclDataType::ACL_INT32, &inputX1);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // create inputX2 aclTensor
    ret = CreateAclTensor(inputX2HostData, inputX2Shape, &inputX2DeviceAddr, aclDataType::ACL_INT32, &inputX2);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    ...
    // 计算workspace大小并申请内存
    ret = aclnnCrossGetWorkspaceSize(inputX1, inputX2, dim, outputY, &workspaceSize, &executor);
