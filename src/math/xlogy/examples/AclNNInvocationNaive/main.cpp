...
int main(int argc, char **argv)
{
    ...
    size_t dataType = 4;
    std::vector<float> inputX1HostData(inputXShape[0] * inputXShape[1]);
    std::vector<float> inputX2HostData(inputXShape[0] * inputXShape[1]);
    ...
    //读取数据
    ReadFile("../input/input_x1.bin", fileSize, *input1, inputXShapeSize_1 * dataType);
    ReadFile("../input/input_x2.bin", fileSize, *input2, inputXShapeSize_1 * dataType);
    INFO_LOG("Set input success");
    // 创建inputX1 aclTensor
    ret = CreateAclTensor(inputX1HostData, inputXShape, &inputX1DeviceAddr, aclDataType::ACL_FLOAT, &inputX1);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建inputX2 aclTensor
    ret = CreateAclTensor(inputX2HostData, inputXShape, &inputX2DeviceAddr, aclDataType::ACL_FLOAT, &inputX2);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);    
    // 创建outputY aclTensor
    ...
    // 计算workspace大小并申请内存
    ret = aclnnXlogyGetWorkspaceSize(inputX1, inputX2, outputY, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnXlogyGetWorkspaceSize failed. ERROR: %d\n", ret); return FAILED);
    ...
    // 执行算子
    ret = aclnnXlogy(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnXlogy failed. ERROR: %d\n", ret); return FAILED);
