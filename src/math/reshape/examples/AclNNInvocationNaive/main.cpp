...
int main(int argc, char **argv)
{
    ...
    std::vector<int64_t> xShape = {8,47,123,1023};
    std::vector<int64_t> shapeShape = {4};
    std::vector<int64_t> yShape = {123,47,8,1023};
    void *xDeviceAddr = nullptr;
    void *shapeDeviceAddr = nullptr;
    ...
    ret = CreateAclTensor(shapeHostData, shapeShape, &shapeDeviceAddr, aclDataType::ACL_INT32, &shape);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    ...
    aclDestroyTensor(x);
    aclDestroyTensor(shape);
    aclDestroyTensor(y);

    // 7. 释放device资源，需要根据具体API的接口定义修改
    aclrtFree(xDeviceAddr);
    aclrtFree(shapeDeviceAddr);
