...
int main(int argc, char **argv)
{
    ...
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    ret = CreateAclTensor(outputYHostData, outShape2, &outputYDeviceAddr, aclDataType::ACL_FLOAT, &outputY);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    ...
    auto size1 = outputXShapeSize;
    std::vector<float> resultData1(size1, 0);
    ret = aclrtMemcpy(resultData1.data(), resultData1.size() * sizeof(resultData1[0]), outputXDeviceAddr,
                      size1 * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return FAILED);
    void ** output1=(void **)(&resultData1);
    //写出数据
    WriteFile("../output/output_x1.bin", *output1, outputXShapeSize * dataType);
    INFO_LOG("Write output success");

    auto size2 = outputYShapeSize;
    std::vector<float> resultData2(size2, 0);
    ret = aclrtMemcpy(resultData2.data(), resultData2.size() * sizeof(resultData2[0]), outputYDeviceAddr,
    size2 * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return FAILED);
    void ** output2=(void **)(&resultData2);
    //写出数据
    WriteFile("../output/output_x2.bin", *output2, outputYShapeSize * dataType);
    INFO_LOG("Write output success");
