## `foreach_minimum_scalar`自定义算子样例说明 
本样例通过`Ascend C`编程语言实现了`foreach_minimum_scalar`算子。
...
<table>
<tr><td rowspan="1" align="center">算子类型(OpType)</td><td colspan="4" align="center">foreach_minimum_scalar</td></tr>
</tr>
...
<tr><td rowspan="1" align="center">核函数名</td><td colspan="4" align="center">foreach_minimum_scalar</td></tr>
</table>
...
### 算子包编译部署
  ...
  - 执行编译
    ...
    ```bash
    bash build.sh -n foreach_minimum_scalar
    ```
...
<table>
    <th>目录</th><th>描述</th>
    <tr>
        <td><a href="./examples/AclNNInvocationNaive"> AclNNInvocationNaive</td><td>通过aclnn调用的方式调用foreach_minimum_scalar算子。</td>
    </tr>
</table>
