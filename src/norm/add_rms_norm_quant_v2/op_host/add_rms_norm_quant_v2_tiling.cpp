...
/*!
 * \file add_rms_norm_quant_v2_tiling.cpp
 * \brief
 */
#include <iostream>
#include "register/op_def_registry.h"
#include "tiling/tiling_api.h"
#include "add_rms_norm_quant_v2_tiling.h"
...
#define VECTOR_INNER_ERR_REPORT_TILIING(op_name, err_msg, ...)                 \
  std::printf(err_msg, ##__VA_ARGS__)
...
constexpr uint32_t UB_FACTOR_B16 = 7168;
constexpr uint32_t UB_FACTOR_B16_CUTD = 6144;
...
constexpr size_t INPUT_IDX_BIAS = 7;
...
static void InitPlatformParams(gert::TilingContext *context,
                               const AddRmsNormQuantV2CompileInfo *ptrCompileInfo,
                               uint32_t &numCore, uint64_t &ubSize,
...
ge::graphStatus GetOpDescInfo(gert::TilingContext *context,
                          uint32_t &hasZeroPoints1, uint32_t &hasBias,
                          uint32_t &numCol, uint32_t &numRow, float &epsilon,
                          float &avgFactor) {
  auto zeroPoints1Desc = context->GetOptionalInputDesc(INPUT_IDX_ZERO_POINTS1);
  hasZeroPoints1 = (zeroPoints1Desc == nullptr) ? 0 : 1;

  auto biasDesc = context->GetOptionalInputDesc(INPUT_IDX_BIAS);
  hasBias = (biasDesc == nullptr) ? 0 : 1;

  const gert::Shape xShape = context->GetInputShape(0)->GetStorageShape();
  const gert::Shape gammaShape =
      context->GetInputShape(INPUT_IDX_GAMMA)->GetStorageShape();
...
static void CalcModeAndUbFactor(gert::TilingContext *context, uint32_t &modeKey,
                                uint32_t &ubFactor, uint32_t blockFactor,
                                platform_ascendc::SocVersion &socVersion,
                                uint32_t numCol, DataType &dataType) {
  if (blockFactor == 1 &&
    ...
    ubFactor =
        (dataType == ge::DT_FLOAT) ? UB_FACTOR_B32_CUTD : UB_FACTOR_B16_CUTD;
...
ge::graphStatus Tiling4AddRmsNormQuantV2(gert::TilingContext *context) {
  AddRmsNormQuantV2TilingData tiling;
  auto ptrCompileInfo = reinterpret_cast<const AddRmsNormQuantV2CompileInfo *>(
  ...
  uint32_t numCol, numRow, hasZeroPoints1, hasBias;
  float epsilon, avgFactor;
  OP_TILING_CHECK(GetOpDescInfo(context, hasZeroPoints1, hasBias, numCol,
                                numRow, epsilon, avgFactor) != SUCCESS,
                  VECTOR_INNER_ERR_REPORT_TILIING(context->GetNodeName(),
                                                  "Get Opdesc Info failed."),
                  return ge::GRAPH_FAILED);
  ...
  CalcModeAndUbFactor(context, modeKey, ubFactor, blockFactor, socVersion,
                      numCol, dataType);
  ...
  OP_LOGD(context->GetNodeName(),
          "Tiling SUCCEEDED. Tiling Key: %u, Block Dim: %u", tilingKey,
          useCoreNum);

  return ge::GRAPH_SUCCESS;
}

ge::graphStatus TilingPrepare4AddRmsNormQuantV2(gert::TilingParseContext *context) {
  auto compileInfo = GetCompileInfoPtr<AddRmsNormQuantV2CompileInfo>(context);
...
IMPL_OP_OPTILING(AddRmsNormQuantV2)
    ...
    .TilingParse<AddRmsNormQuantV2CompileInfo>(TilingPrepare4AddRmsNormQuantV2);
