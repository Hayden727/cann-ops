 ...
 * "AS IS" BASIS, WITHOUT WARRANTIES OF ANY, EITHER EXPRESS OR IMPLIED,
...
/*!
 * \file add_rms_norm_quant_v2_split_d.h
 * \brief Kernel implementation for AddRmsNormQuantV2 operator (SplitD mode).
 */
#ifndef ADD_RMS_NORM_QUANT_V2_SPLIT_D_H_
#define ADD_RMS_NORM_QUANT_V2_SPLIT_D_H_
#include "rms_norm_base.h"

using namespace AscendC;

template <typename TX, typename TScale, typename TOffset>
class KernelAddRmsNormQuantV2SplitD {
public:
  __aicore__ inline KernelAddRmsNormQuantV2SplitD(TPipe *pipe) { Ppipe = pipe; }

  __aicore__ inline void Init(GM_ADDR x1, GM_ADDR x2, GM_ADDR gamma,
                              GM_ADDR scales1, GM_ADDR scales2,
                              GM_ADDR zero_points1, GM_ADDR zero_points2,
    ...
    x1Gm.SetGlobalBuffer((__gm__ TX *)x1 + blockIdx_ * blockFactor * numCol,
                         rowWork * numCol);
    x2Gm.SetGlobalBuffer((__gm__ TX *)x2 + blockIdx_ * blockFactor * numCol,
                         rowWork * numCol);
    ...
    y1Gm.SetGlobalBuffer((__gm__ int8_t *)y1 + blockIdx_ * blockFactor * numCol,
                         rowWork * numCol);
    y2Gm.SetGlobalBuffer((__gm__ int8_t *)y2 + blockIdx_ * blockFactor * numCol,
                         rowWork * numCol);
    xGm.SetGlobalBuffer((__gm__ TX *)x + blockIdx_ * blockFactor * numCol,
                        rowWork * numCol);
  ...
  __aicore__ inline void SubProcess(uint32_t iO, uint32_t calcRowNum,
                                    uint32_t jMax, uint32_t colTail) {
...
private:
  __aicore__ inline void CopyInAndAdd(uint32_t iIdx, uint32_t jIdx,
                                      uint32_t num) {
  ...
  __aicore__ inline void ComputeFormer(uint32_t iOIdx, uint32_t calcRowNum,
                                       uint32_t jIdx,
  ...
  ComputeSum(uint32_t iIIdx, LocalTensor<float> &sumLocal, uint32_t num) {
  ...
  __aicore__ inline void ComputeRstd(LocalTensor<float> rstdLocal,
                                     uint32_t num) {
    LocalTensor<float> reduce_buf_local = reduceFp32Buf.Get<float>();
  ...
  __aicore__ inline void ComputeLatter(uint32_t iOIdx, uint32_t calcRowNum,
                                       uint32_t jIdx,
                                       LocalTensor<float> &rstdLocal,
                                       uint32_t num) {
    CopyInParams(jIdx, num);
    ...
    for (uint32_t i_i = 0; i_i < calcRowNum; i_i++) {
      CopyInAndAdd(iOIdx * rowFactor + i_i, jIdx, num);
      ComputeY(i_i, gammaLocal, biasLocal, rstdLocal, num);
  ...
  __aicore__ inline void CopyInParams(uint32_t jIdx, uint32_t num) {
    ...
    if constexpr (IsSame<TScale, float>::value) {
      ...
      LocalTensor<bfloat16_t> scales1Bf16 =
          scales1Buf.Get<bfloat16_t>()[ubFactor];
    ...
    if (hasZeroPoints1) {
      ...
      if constexpr (IsSame<TOffset, int32_t>::value) {
        ...
        DataCopyCustom<int32_t>(zeroPoints1Int32,
                                zeroPoints1Gm[jIdx * ubFactor], num);
        event_t eventMte2V2 = static_cast<event_t>(
        ...
        LocalTensor<bfloat16_t> zeroPoints1Bf16 =
            zeroPoints1Buf.Get<bfloat16_t>()[ubFactor];
        DataCopyCustom<bfloat16_t>(zeroPoints1Bf16,
                                   zeroPoints1Gm[jIdx * ubFactor], num);
        event_t eventMte2V2 = static_cast<event_t>(
            GetTPipePtr()->FetchEventID(HardEvent::MTE2_V));
        SetFlag<HardEvent::MTE2_V>(eventMte2V2);
    ...
    if (this->hasBias) {
      ...
      DataCopyCustom<TX>(biasLocal, biasGm[jIdx * ubFactor], num);
      inQueueBias.EnQue(biasLocal);
    }
  }

  __aicore__ inline void ComputeY(uint32_t iIIdx, LocalTensor<TX> &gammaLocal,
                                  LocalTensor<TX> &biasLocal,
    ...
    if (this->hasBias) {
      if constexpr (IsSame<TX, half>::value) {
        ...
        Cast(biasFp32, biasLocal, RoundMode::CAST_NONE, num);
        PipeBarrier<PIPE_V>();
        Add(xFp32Local, xFp32Local, biasFp32, num);
      } else { // bfloat16
        LocalTensor<float> biasFp32 = sqxBuf.Get<float>();
        Cast(biasFp32, biasLocal, RoundMode::CAST_NONE, num);
        PipeBarrier<PIPE_V>();
...
#endif // ADD_RMS_NORM_QUANT_V2_SPLIT_D_H_
