...
/*!
 * \file add_rms_norm_quant_v2_single_n.h
 * \brief Kernel implementation for AddRmsNormQuantV2 operator (SingleN mode).
 */
#ifndef ADD_RMS_NORM_QUANT_V2_SINGLE_N_H_
#define ADD_RMS_NORM_QUANT_V2_SINGLE_N_H_
#include "rms_norm_base.h"

using namespace AscendC;

template <typename TX, typename TScale, typename TOffset>
class KernelAddRmsNormQuantV2SingleN {
public:
  __aicore__ inline KernelAddRmsNormQuantV2SingleN(TPipe *pipe) {
    Ppipe = pipe;
  }
  ...
  __aicore__ inline void Init(GM_ADDR x1, GM_ADDR x2, GM_ADDR gamma,
    ...
    Ppipe->InitBuffer(unitBuf, 195584);
  }
...
private:
  __aicore__ inline void ProcessFp16() {
    ...
    DataCopyCustom<TX>(x2Local, gammaGm, numCol);
    SetFlag<HardEvent::MTE2_V>(eventMTE2V2);
    ...
    if (hasZeroPoints1) {
      DataCopyCustom<TOffset>(sqxLocal.ReinterpretCast<TOffset>(),
                              zeroPoints1Gm, numCol);
    }
    SetFlag<HardEvent::MTE2_V>(eventMTE2V3);
    WaitFlag<HardEvent::MTE2_V>(eventMTE2V3);
    Cast(sqxLocal, sqxLocal.ReinterpretCast<TOffset>(), RoundMode::CAST_NONE,
         numCol);
    PipeBarrier<PIPE_V>();
    if (hasZeroPoints1) {
  ...
  __aicore__ inline void ProcessBf16() {
    ...
    if constexpr (IsSame<TScale, bfloat16_t>::value) {
      DataCopyCustom<TScale>(
          tmpLocal.template ReinterpretCast<TScale>()[ubFactor], scales1Gm,
          numCol);
    } else {
      DataCopyCustom<TScale>(tmpLocal.template ReinterpretCast<TScale>(),
                             scales1Gm, numCol);
    }
    event_t eventMTE2V3 =
        static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE2_V));
    ...
    WaitFlag<HardEvent::MTE2_V>(eventMTE2V3);
    if constexpr (IsSame<TScale, bfloat16_t>::value) {
      Cast(tmpLocal, tmpLocal.template ReinterpretCast<TScale>()[ubFactor],
    ...
    if (hasZeroPoints1) {
      if constexpr (IsSame<TOffset, bfloat16_t>::value) {
        DataCopyCustom<TOffset>(sqxLocal.ReinterpretCast<TOffset>()[ubFactor],
                                zeroPoints1Gm, numCol);
      } else {
        DataCopyCustom<TOffset>(sqxLocal.ReinterpretCast<TOffset>(),
                                zeroPoints1Gm, numCol);
    ...
    WaitFlag<HardEvent::MTE2_V>(eventMTE2V3);
    if constexpr (IsSame<TOffset, bfloat16_t>::value) {
      Cast(sqxLocal, sqxLocal.ReinterpretCast<TOffset>()[ubFactor],
           RoundMode::CAST_NONE, numCol);
    } else {
      Cast(sqxLocal, sqxLocal.ReinterpretCast<TOffset>(), RoundMode::CAST_NONE,
           numCol);
...
#endif // ADD_RMS_NORM_QUANT_V2_SINGLE_N_H_
