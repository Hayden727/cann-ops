...
bool OpRunner::RunOp()
{
    ...
    auto ret =
        aclnnAddRmsNormQuantGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2],inputTensor_[3], nullptr,inputTensor_[4], nullptr,-1,1e-6,true,
        outputTensor_[0],outputTensor_[1],outputTensor_[2], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    ret = aclnnAddRmsNormQuant(workspace_, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
