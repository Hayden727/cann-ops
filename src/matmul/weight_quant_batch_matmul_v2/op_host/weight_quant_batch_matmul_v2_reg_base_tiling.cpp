...
bool WeightQuantBatchMatmulV2RegBase::IsCapable() {
    if (matmulInfoPtr_->antiQuantType == QuantType::PER_TENSOR) {
        OP_LOGI(opName_, "the reg base template doesn't support the per-tensor mode");
        return false;
    }

    if (matmulInfoPtr_->bDtype != ge::DT_INT4 && matmulInfoPtr_->bDtype != ge::DT_INT8) {
        OP_LOGI(opName_, "the reg base template only support weight dtype int4 or int8, but is [%s]",
                ge::TypeUtils::DataTypeToAscendString(matmulInfoPtr_->bDtype).GetString());
        return false;
    }

    if (matmulInfoPtr_->antiQuantType == QuantType::PER_CHANNEL) {
        OP_LOGI(opName_, "the reg base template doesn't support the per-channel mode");
        return false;
    }
...
void WeightQuantBatchMatmulV2RegBase::GetBubTilingA16W8Trans(int64_t &nBubSize, int64_t &kBubSize) const
{
    ...
    if (nBl1Size * kBl1Size <= MTE2_MIN_LOAD_SIZE_V100 &&
        ...
        nBubSize = nBl1Size;
        kBubSize = kBl1Size;
        OP_LOGD(opName_, "No need to split BL1, set nBubSize: %ld, kBubSize: %ld", nBubSize, kBubSize);
        return;
    }
    ...
    if (kBl1Size % CACHE_LINE_V100 > 0) {
        OP_LOGD(opName_, "kBl1 is not %ld aligned, use default UB tiling. nBubSize: %ld, kBubSize: %ld",
    ...
    for (int64_t tmpKBub = kBl1Size; tmpKBub >= CACHE_LINE_V100; tmpKBub -= CACHE_LINE_V100) {
        for (int64_t tmpNBub = BLOCK_CUBE; tmpNBub <= nBl1Size; tmpNBub += BLOCK_CUBE) {
            ...
            if (kBl1Size % tmpKBub == 0 && nBl1Size % tmpNBub == 0) {
                nBubSize = tmpNBub;
                kBubSize = tmpKBub;
                OP_LOGD(opName_, "Find ideal UB tiling, nBubSize: %ld, kBubSize: %ld", nBubSize, kBubSize);
                return;
            }
        }
    }
    OP_LOGD(opName_, "Use default UB tiling. nBubSize: %ld, kBubSize: %ld", nBubSize, kBubSize);
}
