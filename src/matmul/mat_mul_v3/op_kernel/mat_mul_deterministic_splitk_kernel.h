                                           ...
                                           uint64_t singleCoreN, uint64_t n, uint64_t cnt, bool orderFlag)
{
    ...
    CopyGmToUbufAlign<float>((__ubuf__ float *)ubSrc1.GetPhyAddr(),
                             (__gm__ float *)gmSrc[gmOffset].GetPhyAddr(),
                             0, burst, burstLen * sizeof(float), 0, 0, srcGap * sizeof(float), dstGap * sizeof(float) / 32);

    for (uint64_t j = 1; j < singleCoreNum; ++j) {
        tmpOffset += (singleSize << 1);
        CopyGmToUbufAlign<float>((__ubuf__ float *)ubSrc2.GetPhyAddr(), (__gm__ float *)gmSrc[gmOffset +
                                 tmpOffset].GetPhyAddr(),
                                 0, burst, burstLen * sizeof(float), 0, 0, srcGap * sizeof(float), dstGap * sizeof(float) / 32);
    ...
    if (orderFlag) {
        ...
        if constexpr (sizeof(T) == sizeof(half)) {
            CopyUbufToGmAlign<T>((__gm__ T *)gmDst[dstOffset].GetPhyAddr(), (__ubuf__ T *)ubDst.GetPhyAddr(), 0,
                                burst, burstLen * sizeof(T), 0, 0, srcGap * sizeof(T) / 32, dstGap * sizeof(T)); // 32 is blocksize

        } else if constexpr (sizeof(T) == sizeof(float)) {
            CopyUbufToGmAlign<T>((__gm__ T *)gmDst[dstOffset].GetPhyAddr(), (__ubuf__ T *)ubSrc1.GetPhyAddr(), 0,
                                burst, burstLen * sizeof(T), 0, 0, srcGap * sizeof(T) / 32, dstGap * sizeof(T)); // 32 is blocksize
        }
    } else {
        if constexpr (sizeof(T) == sizeof(half)) {
            CopyUbufToGmAlign<T>((__gm__ T *)gmDst[gmOffset +
                                    index * singleSize].GetPhyAddr(), (__ubuf__ T *)ubDst.GetPhyAddr(), 0, 1,
                                    dataSizeToMove * sizeof(T), 0, 0, 0, 0);
        } else if constexpr (sizeof(T) == sizeof(float)) {
            CopyUbufToGmAlign<T>((__gm__ T *)gmDst[gmOffset +
                                    index * singleSize].GetPhyAddr(), (__ubuf__ T *)ubSrc1.GetPhyAddr(), 0, 1,
                                    dataSizeToMove * sizeof(T), 0, 0, 0, 0);
        }
    }
}
                                                   ...
                                                   const TCubeTiling& tiling, bool isBias)
{
    ...
    PipeBarrier<PIPE_ALL>();
    SetAtomicNone();
    orderFlag ? mmnk.SetHF32(false, 0) : mmmk.SetHF32(false, 0);
}
