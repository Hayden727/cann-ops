...
namespace ops {
template <typename T>
static ge::graphStatus PadV4GradInfershape(const gert::InferShapeContext* context, const gert::Shape* x_shape,
...
static ge::graphStatus InferShapePadV4Grad(gert::InferShapeContext* context) {
    ...
    switch (paddings_dtype) {
        case ge::DT_INT32: {
            return PadV4GradInfershape<int32_t>(context, x_shape, paddings_tensor, y_shape);
        }
        case ge::DT_INT64: {
            return PadV4GradInfershape<int64_t>(context, x_shape, paddings_tensor, y_shape);
        }
        default:
            return ge::GRAPH_FAILED;
    }
...
IMPL_OP_INFERSHAPE(PadV4Grad)
    .InferShape(InferShapePadV4Grad)
