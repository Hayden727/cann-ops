...
int main() {
    ...
    size_t gradOutputShapeSize = GetShapeSize(gradOutputShape);
    ...
    std::vector<float> gradOutputHostData(gradOutputShapeSize);
    std::vector<float> selfHostData(selfShapeSize);
    std::vector<float> gradInputHostData(selfShapeSize);
    std::vector<int64_t> paddingData = {1, 1, 1, 1};
    void** inputgradOutput = (void**)(&gradOutputHostData);
    void** inputSelf = (void**)(&selfHostData);
    size_t dtypeSize = sizeof(float);

    size_t fileSize;

    ReadFile("../input/input_grad_output.bin", fileSize, *inputgradOutput, gradOutputShapeSize * dtypeSize);
    ...
    int64_t paddingDim = 4;
    padding = aclCreateIntArray(paddingData.data(), paddingDim);
    ...
    WriteFile("../output/output_out.bin", *output_out, size * sizeof(float));
