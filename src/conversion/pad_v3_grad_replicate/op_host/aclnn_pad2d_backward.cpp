...
#include "src/conversion/circular_pad_grad/op_host/padv3grad.h"
...
static bool CheckPaddingValue(const aclIntArray *padding, const string &mode) {
                        ...
                        const string& mode, const aclTensor *gradInput)
{
    ...
    OP_CHECK(padding->Size() == 4,
                OP_LOGE(ACLNN_ERR_PARAM_INVALID, "padding length should be 4, but got %zu.",
                        padding->Size()),
                return false);
    ...
    if (gradOutput->GetDataType() == op::DataType::DT_BF16) {
        ...
        OP_CHECK(isBF16Support, OP_LOGE(ACLNN_ERR_PARAM_INVALID,
...
{
// 1. 检查参数是否为空指针
CHECK_RET(MY_API::CheckNotNull(gradOutput, self, padding, gradInput), ACLNN_ERR_PARAM_NULLPTR);

// 2. 检查输入的数据类型是否在API支持的数据类型范围之内，需要根据api定义校验
CHECK_RET(MY_API::CheckDtypeValid(gradOutput, self, gradInput), ACLNN_ERR_PARAM_INVALID);

// 3. 检查数据格式是否支持
CHECK_RET(MY_API::CheckFormat(gradOutput, self, gradInput), ACLNN_ERR_PARAM_INVALID);

// 4. 检查shape是否满足约束
CHECK_RET(MY_API::CheckShape(gradOutput, self, padding, mode, gradInput), ACLNN_ERR_PARAM_INVALID);
...
static aclnnStatus commonPad2dBackward(const aclTensor *gradOutput, const aclTensor *self,
                                       const aclIntArray *padding, const std::string& mode, aclTensor *gradInput,
                                       uint64_t *workspaceSize, aclOpExecutor **executor)
{
    ...
    auto padFlag = MY_API::CheckPaddingValue(padding, mode);
    const aclTensor *pad2dbackwardResult = nullptr;
    auto originOutDataType = gradOutput->GetDataType();
    // cast to fp32
    if (mode == MY_API::REPLICATION_MODE && padFlag &&
...
aclnnStatus aclnnReflectionPad2dBackwardGetWorkspaceSize(const aclTensor *gradOutput, const aclTensor *self,
    const aclIntArray *padding, aclTensor *gradInput, uint64_t *workspaceSize, aclOpExecutor **executor)
{
    ...
    return commonPad2dBackward(gradOutput, self, padding, MY_API::REFLECTION_MODE, gradInput, workspaceSize, executor);
    }
...
aclnnStatus aclnnReplicationPad2dBackwardGetWorkspaceSize(const aclTensor *gradOutput, const aclTensor *self,
    const aclIntArray *padding, aclTensor *gradInput, uint64_t *workspaceSize, aclOpExecutor **executor)
{
    OP_CHECK_COMM_INPUT(workspaceSize, executor);

    L2_DFX_PHASE_1(aclnnReplicationPad2dBackward, DFX_IN(gradOutput, self, padding), DFX_OUT(gradInput));
    return commonPad2dBackward(gradOutput, self, padding, MY_API::REPLICATION_MODE, gradInput, workspaceSize, executor);
    }

aclnnStatus aclnnReplicationPad2dBackward(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor,
    const aclrtStream stream)
{
