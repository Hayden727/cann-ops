...
class DynamicQuantBase {
 ...
 protected:
  ...
  uint32_t lenHead;
  uint32_t lenTail;
  uint32_t lenMultiRow;
  uint32_t lenGMMultiRow;
  uint32_t outLen;
  uint32_t outInt8PaddingLen;
  uint32_t outInt4PaddingLen;
  uint32_t outLenHead;
  uint32_t outLenTail;
  uint32_t originOutSize;
  uint32_t loopCnt = 0;
  ...
  AscendC::TBuf<AscendC::TPosition::VECCALC> tempScale;
