...
int main()
{
    ...
    ret = aclnnUpsampleTrilinear3dBackwardGetWorkspaceSize(
        gradOut, outputSize, inputSize, alignCoreners, scalesD, scalesH, scalesW, gradInput, &workspaceSize, &executor);
    CHECK_RET(
        ret == ACL_SUCCESS, LOG_PRINT("aclnnUpsampleTrilinear3dBackwardGetWorkspaceSize failed. ERROR: %d\n", ret);
        return ret);
