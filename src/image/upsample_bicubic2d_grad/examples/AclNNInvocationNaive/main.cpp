...
int main()
{
    ...
    // 设定数据
    size_t fileSize;
    ReadFile("../input/output_grad.bin", fileSize, *inputSelf, selfShapeSize * dtypeSize);
    ...
    // 调用aclnnUpsampleBicubic2dGrad第一段接口
    ret = aclnnUpsampleBicubic2dBackwardGetWorkspaceSize(self, outputSize, inputSize, alignCoreners, scalesH, scalesW, out, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnUpsampleBicubic2dGradGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 调用aclnnUpsampleBicubic2dGrad第二段接口
    ret = aclnnUpsampleBicubic2dBackward(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnUpsampleBicubic2dGrad failed. ERROR: %d\n", ret); return ret);
