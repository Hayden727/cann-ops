...
uint32_t UpsampleLinear1dTiling::GetNeedCoreNum(const uint32_t coreNumPlatForm, const uint8_t mode)
{
...
void UpsampleLinear1dTiling::GetTCubeTilingW()
{
...
void UpsampleLinear1dTiling::GetTCubeTilingH()
{
    auto mmDataType = static_cast<matmul_tiling::DataType>(dataType);
...
void UpsampleLinear1dTiling::setSlideSize(const uint32_t coreNumPlatForm, const uint8_t mode)
{
...
inline int64_t UpsampleLinear1dTiling::calculateSlideSize(const uint32_t coreNumPlatForm, uint8_t direction)
{
...
template <typename T1, typename T2>
inline T1 UpsampleLinear1dTiling::CeilA2B(T1 a, T2 b) const
{
    if (b != 0) {
        return (a + b - 1) / b;
    } else {
...
uint64_t UpsampleLinear1dTiling::GetDataTypeVal()
{
    switch (dataType) {
        case ge::DT_FLOAT:
...
static ge::graphStatus tilingPrepareTiling(gert::TilingParseContext *context)
{
    ...
    OP_TILING_CHECK(compileInfo->coreNum <= 0,
        VECTOR_INNER_ERR_REPORT_TILIING(
            context->GetNodeName(), "UpsampleLinear1d GetHardwareInfo Failed, vectorCoreNum:%d",
            compileInfo->coreNum),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
