## `UpsampleNearest3dGrad`自定义算子样例说明 
本样例通过`Ascend C`编程语言实现了`UpsampleNearest3dGrad`算子。

### 算子描述
对由多个输入通道组成的输入信号应用最近邻插值算法进行上采样的反向。

### 算子规格描述

<table>
<tr><td rowspan="1" align="center">算子类型(OpType)</td><td colspan="4" align="center">UpsampleNearest3dGrad</td></tr>
</tr>
...
<tr><td rowspan="1" align="center">核函数名</td><td colspan="4" align="center">upsample_nearest3d_grad</td></tr>
</table>
...
### 算子包编译部署
  ...
  - 执行编译
    ...
    ```bash
    bash build.sh -n upsample_nearest3d_grad
    ```
...
<table>
    <th>目录</th><th>描述</th>
    <tr>
        <td><a href="./examples/AclNNInvocationNaive"> AclNNInvocationNaive</td><td>通过aclnn调用的方式调用UpsampleNearest3dGrad算子。</td>
    </tr>
</table>
