...
```
├── msopst.ini                    // st测试配置文件 
├── Greater_case_fp_type.json       // 测试用例定义文件示例
└── test_greater.py                 // 算子期望数据生成脚本
```
...
## 执行测试用例
  **请确保已根据算子包编译部署步骤完成本算子的编译部署动作。**
  ...
  - 进入到测试用例目录
    ...
    ```bash
    cd ${git_clone_path}/cann-ops/src/math/greater/tests/st
    ```
  ...
  - 执行测试用例
    ...
    ```bash
    ${INSTALL_DIR}/python/site-packages/bin/msopst run -i ./Greater_case_fp_type.json -soc {Soc Version} -out ./output -conf msopst.ini
    ${INSTALL_DIR}/python/site-packages/bin/msopst run -i ./Greater_case_int_type.json -soc {Soc Version} -out ./output -conf msopst.ini
    ```