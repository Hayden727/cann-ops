## `EdgeSubDilateVerC1Threshol8u`自定义算子样例说明 
本样例通过`Ascend C`编程语言实现了`EdgeSubDilateVerC1Threshol8u`算子。

### 算子描述
`EdgeSubDilateVerC1Threshol8u`算子对图像数据进行膨胀操作，然后进行阈值处理，将非零像素值设置为 1，零像素值保持为 0。
...
<table>
    <th>目录</th><th>描述</th>
    <tr>
        <td><a href="./examples/AclNNInvocationNaive"> AclNNInvocationNaive</td><td>通过aclnn调用的方式调用EdgeSubDilateVerC1Threshol8u算子。</td>
    </tr>
</table>
