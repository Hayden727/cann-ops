...
int main(int argc, char **argv)
{
    ...
    std::vector<int64_t> inputYShape = {3440, 4887};
    ...
    std::vector<uint8_t> inputYHostData(inputXShapeSize);
    ...
    // 创建inputY aclTensor
    ret = CreateAclTensor(inputYHostData, inputYShape, &inputYDeviceAddr, aclDataType::ACL_UINT8, &inputY);
    CHECK_RET(ret == ACL_SUCCESS, return FAILED);
    // 创建outputZ aclTensor
    ...
    // 计算workspace大小并申请内存
    ret = aclnnEdgeSubGetWorkspaceSize(inputX, inputY, outputZ, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnEdgeSubGetWorkspaceSize failed. ERROR: %d\n", ret); return FAILED);
    ...
    // 执行算子
    ret = aclnnEdgeSub(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnEdgeSub failed. ERROR: %d\n", ret); return FAILED);
