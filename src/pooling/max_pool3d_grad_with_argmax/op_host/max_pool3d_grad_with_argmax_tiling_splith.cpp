...
/*!
 * \file max_pool3d_grad_with_argmax_tiling_splith.cpp
 * \brief
 */
...
namespace TilingKeys {
    ...
    //no overlap (1), splitD=1, splitH=1, splitW=0, splitKernel = 0, dtype=half
    constexpr uint64_t TILING_KEY_SPLITH_HALF_NO_OVERLAP = 111001;
    //overlap (2), splitD=1, splitH=1, splitW=0, splitKernel = 0, dtype=half
    constexpr uint64_t TILING_KEY_SPLITH_HALF_OVERLAP = 211001;
...
bool MaxPool3DGradWithArgmaxSplitHTiling::IsCapable() {
    splitData.partD = 1;
    splitData.partOutD = inputData.dilation[D_DIM] * (inputData.kernelSize[D_DIM] - 1) + 1;
    std::array<uint64_t, DHW_DIMS> parts{splitData.partD, inputData.inputShape[H_DIM], inputData.inputShape[W_DIM]};
    std::array<uint64_t, DHW_DIMS> partOuts{splitData.partOutD, padOutputData.padOutputShape[H_DIM], padOutputData.padOutputShape[W_DIM]};
    ...
    return ((splitData.partH != 0) && (splitData.partOutH != 0) && (!defaultDilation));
}

uint64_t MaxPool3DGradWithArgmaxSplitHTiling::GetTilingKey() const {
    if (dtype == ge::DataType::DT_FLOAT) {
        return TilingKeys::TILING_KEY_SPLITH_FLOAT;
    } else if (dtype == ge::DataType::DT_FLOAT16) {
        if (!inputData.isOverlap) {
            return TilingKeys::TILING_KEY_SPLITH_HALF_NO_OVERLAP;
        } else {
            return TilingKeys::TILING_KEY_SPLITH_HALF_OVERLAP;
        }
    } else {
        return TilingKeys::TILING_KEY_SPLITH_BF16;
    }
}

void MaxPool3DGradWithArgmaxSplitHTiling::DoUBTiling() {
...
void MaxPool3DGradWithArgmaxSplitHTiling::SetTilingData() {
...
ge::graphStatus MaxPool3DGradWithArgmaxSplitHTiling::DoOpTiling() {
    DoUBTiling();
    SetTilingData();
...
ge::graphStatus MaxPool3DGradWithArgmaxSplitHTiling::PostTiling() {
...
REGISTER_TILING_TEMPLATE("MaxPool3DGradWithArgmax", MaxPool3DGradWithArgmaxSplitHTiling, 4);
