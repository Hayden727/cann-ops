...
class MaxPoolGradWithArgScatter : public MaxPoolGradWithArgScatterBase<TX, TGrad, TArgmax, TY> {
 public:
  __aicore__ inline MaxPoolGradWithArgScatter(TPipe *pipe) : MaxPoolGradWithArgScatterBase<TX, TGrad, TArgmax, TY>(pipe){
  }
  ...
  __aicore__ inline void CalcOutOffset() {
    ...
    for (uint64_t ncIdx = 0; ncIdx < this->block_.ncShape; ncIdx++) {
      ...
      for (uint64_t dhwIdx = 0; dhwIdx < basedhwLen; dhwIdx++) {
        ...
        event_t eventS2MTE3 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::S_MTE3));
        set_flag(PIPE_S, PIPE_MTE3, eventS2MTE3);
        wait_flag(PIPE_S, PIPE_MTE3, eventS2MTE3);
        DataCopyPad(this->yGm[outOffset], yUb, {1, static_cast<uint16_t>(sizeof(TGrad)), 0, 0});
        event_t eventMTE32S = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE3_S));
        set_flag(PIPE_MTE3, PIPE_S, eventMTE32S);
        wait_flag(PIPE_MTE3, PIPE_S, eventMTE32S);
      }
    ...
    for (uint64_t i = 0; i < this->params_.ncRealRound; i++) {
      if (ncIndex < this->params_.ncCnt) {
        ...
        for (uint64_t j = 0; j < this->params_.doCnt; j++) {
          ...
          this->block_.doShape =
              this->block_.doCntIndex >= (this->params_.doCnt - 1) ? this->params_.doTail : this->params_.baseDo;
          for (uint64_t k = 0; k < this->params_.hoCnt; k++) {
            this->block_.hoCntIndex = k;
            this->block_.hoShape =
                this->block_.hoCntIndex >= (this->params_.hoCnt - 1) ? this->params_.hoTail : this->params_.baseHo;
            for (uint64_t l = 0; l < this->params_.woCnt; l++) {
              this->block_.woCntIndex = l;
              this->block_.woShape =
                  this->block_.woCntIndex >= (this->params_.woCnt - 1) ? this->params_.woTail : this->params_.baseWo;
