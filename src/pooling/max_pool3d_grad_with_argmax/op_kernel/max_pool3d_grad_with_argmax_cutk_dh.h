...
__aicore__ inline void Img2ColPartKDH(const LocalTensor<TArgmax>& indexColUb,
...
__aicore__ inline void Col2ImgPartKDH(const LocalTensor<float>& yTranUb,
...
__aicore__ inline void CalcGradSubProcessKDH(const LocalTensor<TArgmax>& indexImgUb,
...
template<typename T>
__aicore__ inline void  CalcGradKDH(const GlobalTensor<T>& dstGm,
                                    const LocalTensor<TArgmax>& argmaxTranUb,
                                    const LocalTensor<TGrad>& gradTranUb,
                                    uint64_t kdIdx,
                                    uint64_t khIdx)
{
    ...
    if (dataValid) {
        ...
        genIndicesParams.firstValue =
            ((this->block_.doCntIndex * this->params_.baseDo) * this->params_.sd +
            kdIdx - this->params_.padDTop) * this->params_.hiDim * this->params_.wiDim +
            ((this->block_.hoCntIndex * this->params_.baseHo) * this->params_.sh +
            khIdx - this->params_.padHTop) * this->params_.wiDim +
