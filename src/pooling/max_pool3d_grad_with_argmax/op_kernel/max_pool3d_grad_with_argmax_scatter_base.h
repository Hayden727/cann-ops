...
class MaxPoolGradWithArgScatterBase {
 public:
  ...
  __aicore__ inline void InitParams(const MaxPool3DGradWithArgmaxTilingData* tiling) {
    ...
    params_.realRound =
        params_.ncRealRound * params_.doCnt * params_.hoCnt * params_.woCnt;  // 处理的base矩阵个数，包括base矩阵和tail矩阵
  }
    ...
    if (params_.baseWo == params_.woDim) {
      ...
      if (baseblockLen > UNIT_BLOCK_LEN && baseblockLen % UNIT_BLOCK_LEN == 0) {
        ...
        if (tailBlockLen != 0) {
          ...
          padGrad.isPad = true;
          DataCopyPad(gradUb[baseBlockCount * (UNIT_BLOCK_LEN / sizeof(TGrad))],
    ...
    if (params_.baseWo == params_.woDim) {
      ...
      if (baseblockLen > UNIT_BLOCK_LEN && baseblockLen % UNIT_BLOCK_LEN == 0) {
        ...
        if (tailBlockLen != 0) {
          ...
          DataCopyPad(argmaxUb[baseBlockCount * (UNIT_BLOCK_LEN / sizeof(TArgmax))],
