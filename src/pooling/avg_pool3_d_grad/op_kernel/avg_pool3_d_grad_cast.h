...
private:
    ...
    __aicore__ inline void CopyOut(int64_t d, int64_t h, int64_t w, int64_t nc, int64_t ubIndex);
...
__aicore__ inline void KernelAvgPool3DGradCast<T>::Init(GM_ADDR grads, GM_ADDR output, const AvgPool3dGradTilingParam &tilingData, GM_ADDR workspace) {
    ...
    this->pipe.InitBuffer(this->xInBuf, this->nLine * this->ncAlign * sizeof(T));
    this->pipe.InitBuffer(this->calcBuffer, this->nLine * this->ncAlign * sizeof(float));
    ...
    if (!this->kernelsOverlap) {
        this->pipe.InitBuffer(this->yOutBuf, this->nLine * this->ncAlign * sizeof(T));
...
__aicore__ inline void KernelAvgPool3DGradCast<T>::ProcessAndCopyout() {
    auto processStart = this->normalCoreNCNum * this->thisCoreIdx;
...
__aicore__ inline void KernelAvgPool3DGradCast<T>::ProcessPerLine(int64_t index, int64_t ubIndex) {
    ...
    this->CopyIn(this->indexD, this->indexH, this->indexW, this->indexNC, ubIndex);
    ...
    for (auto iD = this->dStart; iD < this->dEnd; ++iD) {
        for (auto iH = this->hStart; iH < this->hEnd; ++iH) {
            for (auto iW = this->wStart; iW < this->wEnd; ++iW) {
                this->CopyOut(iD, iH, iW, this->indexNC, ubIndex);
            }
        }
    }
}
...
__aicore__ inline void KernelAvgPool3DGradCast<T>::Compute(int64_t ubIndex) {
    auto offset = ubIndex * this->ncAlign;
...
template <typename T>
__aicore__ inline void KernelAvgPool3DGradCast<T>::CopyOut(int64_t d, int64_t h, int64_t w, int64_t nc, int64_t ubIndex) {
    auto srcUbOffset = ubIndex * this->ncAlign;
    auto dstGmOffset = d * (this->inH * this->inW * this->ncTotal) + 
                        h * (this->inW * this->ncTotal) + 
                        w * this->ncTotal + 
                        nc * this->ncCount;
    if (!this->kernelsOverlap) {
