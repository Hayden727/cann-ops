...
int main(int argc, char **argv)
{
    ...
    size_t gradOutputSize = GetShapeSize(gradOutputShape);
    size_t selfShapeSize = GetShapeSize(selfShape);
    size_t outputShapeSize = GetShapeSize(outputShape);

    std::vector<float> gradOutputHostData(gradOutputSize);
    std::vector<float> selfHostData(selfShapeSize);
    std::vector<float> outputHostData(outputShapeSize);
    size_t dataType = sizeof(float);
    size_t fileSize = 0;

    void ** gradOutputHostDataVoid=(void **)(&gradOutputHostData);
    void ** selfHostDataVoid=(void **)(&selfHostData);

    //读取数据
    ReadFile("../input/grads.bin", fileSize, *gradOutputHostDataVoid, gradOutputSize * dataType);
    ReadFile("../input/input.bin", fileSize, *selfHostDataVoid, selfShapeSize * dataType);
    ...
    void ** out1=(void **)(&resultData);
    //写出数据
    WriteFile("../output/out.bin", *out1, outputShapeSize * dataType);
